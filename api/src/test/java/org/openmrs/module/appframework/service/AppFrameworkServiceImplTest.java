...
public class AppFrameworkServiceImplTest extends BaseModuleContextSensitiveTest {
	...
	public void setUp() throws Exception {
		...
		featureToggles.setPropertiesFile(new File(this.getClass()
				.getResource("/" + FeatureToggleProperties.FEATURE_TOGGLE_PROPERTIES_FILE_NAME).getFile()));

		app1 = new AppDescriptor("app1", "desc1", "label1", "url1", "iconUrl", "tinyIconUrl", 20, null,
				Arrays.asList(new ExtensionPoint("extensionPoint1"), new ExtensionPoint("extensionPoint2")));

		app2 = new AppDescriptor("app2", "desc1", "label1", "url1", "iconUrl", "tinyIconUrl", 10, null,
	...
	public void testGetAllEnabledAppsShouldCorrectlyHandleNegatedFeatureToggles() throws Exception {
		...
		// we change these so that app1 should only be enabled if app toggle 1 is not
		// enabled, same for app2
		app1.setFeatureToggle("!app1Toggle");
		app2.setFeatureToggle("!app2Toggle");
	...
	public void testCheckRequireExpression() throws Exception {
		...
		assertFalse(
				service.checkRequireExpression(extensionRequiring("visit.admitted && visit.admitted"), contextModel));
	}
	...
	public void testCheckRequireExpressionWithMapProperty() throws Exception {
		...
		AppFrameworkServiceImpl service = new AppFrameworkServiceImpl(null, null, null, null, null, null, null, null);
		assertTrue(
				service.checkRequireExpression(extensionRequiring("sessionLocation.uuid == 'abc-123'"), contextModel));
	}
	...
	public void testUtilityFunctionForRequireExpressions() throws Exception {
		...
		AppFrameworkServiceImpl service = new AppFrameworkServiceImpl(null, null, null, null, null, null, null, null);
		assertTrue(service.checkRequireExpression(
				extensionRequiring("hasMemberWithProperty(sessionLocation.tags, 'display', 'Login Location')"),
				contextModel));
		assertFalse(service.checkRequireExpression(
				extensionRequiring("hasMemberWithProperty(sessionLocation.tags, 'display', 'Not this tag')"),
				contextModel));
	}
