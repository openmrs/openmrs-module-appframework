...
public class ProgramConfiguration {
	...
	public boolean hasValidProgramTree() {
		...
		if (program == null && workflow != null && state != null) {
			return workflow.getSortedStates().contains(state);
		}
		...
		// For cases where a workflow and program were specified, be sure the workflow belongs to the specified workflow
		if (program != null && workflow != null) {
			...
			if (state != null) {
				...
				return programHasWorkflow && workflow.getSortedStates().contains(state);
			}
