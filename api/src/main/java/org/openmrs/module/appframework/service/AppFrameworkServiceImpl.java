...
public class AppFrameworkServiceImpl extends BaseOpenmrsService implements AppFrameworkService {
	...
	public AppFrameworkServiceImpl(AllAppTemplates allAppTemplates, AllAppDescriptors allAppDescriptors,
			...
			AppFrameworkConfig appFrameworkConfig, AllUserApps allUserApps) throws ScriptException {
		...
		// there is surely a cleaner way to define this utility function in the global
		// scope
		this.javascriptEngine.eval("function hasMemberWithProperty(list, propName, val) { "
				+ "if (!list) { return false; } " + "var i, len=list.length; " + "for (i=0; i<len; ++i) { "
				+ "  if (list[i][propName] == val) { return true; } " + "} " + "return false; " + "}");
		Object hasMemberWithProperty = javascriptEngine.getBindings(ScriptContext.ENGINE_SCOPE)
				.get("hasMemberWithProperty");
		...
		for (AppDescriptor app : getAllEnabledApps()) {
			if (app.getExtensions() != null) {
				for (Extension extension : app.getExtensions()) {
					// extensions that belong to apps can't be disabled independently of their app,
					// so we don't check AllComponentsState here
	...
	private boolean disabledByComponentState(Extension extension) {
		ComponentState componentState = allComponentsState.getComponentState(extension.getId(),
	...
	public boolean checkRequireExpression(Extension candidate, AppContextModel contextModel) {
		try {
			...
			if (StringUtils.isBlank(requireExpression)) {
				...
				// If any properties in contextModel are Maps, we want to allow scripts to
				// access their subproperties
				// with dot notation, but ScriptEngine and Bindings don't naturally handle this.
				// Instead we will convert
				// all Map-type properties to JSON and use this to define objects directly
				...
				for (Map.Entry<String, Object> e : mapProperties.entrySet()) {
					try {
						javascriptEngine
								.eval("var " + e.getKey() + " = " + jackson.writeValueAsString(e.getValue()) + ";");
					} catch (Exception ex) {
						...
						if (e.getValue() instanceof Map) {
							...
							for (Map.Entry entry : map.entrySet()) {
								extraInfo.append(" ").append(entry.getKey().toString()).append(":").append(
										entry.getValue() == null ? "null" : entry.getValue().getClass().toString());
							}
						}
						log.error("Failed to set '" + e.getKey() + "' scope variable (" + extraInfo
								+ ") while evaluating require check for extension " + candidate.getId(), ex);
						return false;
					}
				}
	...
	public List<Location> getLoginLocations() {
		LocationTag supportsLogin = locationService
				.getLocationTagByName(AppFrameworkConstants.LOCATION_TAG_SUPPORTS_LOGIN);
