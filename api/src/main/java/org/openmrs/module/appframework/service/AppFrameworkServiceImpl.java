...
public class AppFrameworkServiceImpl extends BaseOpenmrsService implements AppFrameworkService {
            ...
            if (StringUtils.isBlank(requireExpression)) {
                ...
                for (Map.Entry<String, Object> e : mapProperties.entrySet()) {
                    try {
                        ...
                        log.error("Failed to set '" + e.getKey() + "' scope variable (" + extraInfo + ") while evaluating require check for extension " + candidate.getId(), ex);
                        return false;
                    }
                }
	...
	public List<Extension> getAllExtensions() {
		...
		List<Extension> allExtensions = new ArrayList<Extension>();
